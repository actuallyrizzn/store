...
All carry `.status_code` and `.response_body` where applicable. Connection and timeout errors (e.g. no server) are raised as `requests.exceptions.ConnectionError` / `requests.exceptions.Timeout`.
